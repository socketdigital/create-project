...
class Files {
  ...
  static getProjectName(targetDir) {
    return targetDir === "." ? path$3.basename(path$3.resolve()) : targetDir;
  }
  ...
  static getFullDirPath(dirName) {
    ...
    return path$3.resolve(currentDir, dirName);
  }
  static makeDir(fullDirPath) {
    fs$1.mkdirSync(fullDirPath, { recursive: true });
  }
}
...
class Args {
  ...
  parse() {
    const args = minimist$1(process.argv.slice(2), this.options);
...
class Prompts {
  ...
  async handlePrompts() {
    ...
    this.prompts = await prompts$1(this.questions, { onCancel });
    if (wasCancelled) {
      process.exit(1);
    }
  }
...
class CreateProject {
  ...
  _createProject(
    ...
    const templateSourceDir = path$3.join(
  ...
  getProjectDirname() {
    ...
    const parentDir = path$3.resolve(dirname, "..");
    return parentDir;
  }
  ...
  copyTemplateDirectory(templateSourceDir, projectPath, templateName) {
    console.log(
      "\n",
      $.blue(`Creating ${templateName}`),
      "\n",
      $.blue(`in ${projectPath}`)
    );

    fs$1.cpSync(templateSourceDir, projectPath, { recursive: true });

    fs$1.renameSync(
      path$3.join(projectPath, "gitignore"),
      path$3.join(projectPath, ".gitignore")
    );
  }
  ...
  copySourceFolders(sourceDir, sourceFolders, projectPath) {
    sourceFolders.forEach((folder) => {
      const sourceFolder = path$3.join(sourceDir, folder);
      const destinationFolder = path$3.join(projectPath, folder);
      fs$1.cpSync(sourceFolder, destinationFolder, { recursive: true });
    });
  }
  ...
  writePackageJson(
    ...
    const packageJson = JSON.parse(
      fs$1.readFileSync(path$3.join(templateSourceDir, `package.json`), "utf-8")
    );
    ...
    fs$1.writeFileSync(
      path$3.join(projectPath, "package.json"),
  ...
  installPackageJson(projectPath) {
    const npm = spawn$1.sync("npm", ["install"], {
      cwd: projectPath,
      stdio: "inherit",
    });
    if (npm.status !== 0) {
